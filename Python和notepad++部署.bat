...
powershell -Command "[System.Net.WebClient]::new().DownloadFile('https://github.com/notepad-plus-plus/notepad-plus-plus/releases/download/v8.8.5/npp.8.8.5.portable.x64.zip', '%cd%\npp.8.8.5.portable.x64.zip')"
powershell -Command "Expand-Archive -Path "%cd%\npp.8.8.5.portable.x64.zip" -DestinationPath "%cd%\npp.8.8.5.portable.x64" -Force"
md "%cd%\npp.8.8.5.portable.x64\data\tmp"
start "" /i "%cd%\npp.8.8.5.portable.x64\notepad++.exe"
)
