...
start "" /i "%cd%\vim\vim91\gvim.exe" || (

rem 注意要手动开启 【语法-自动】，然后开启【语法-在菜单中显示文件类型】，否则不起效。
powershell -Command "[System.Net.WebClient]::new().DownloadFile('https://www.vim.org/downloads/gvim_9.1.0821_x64.zip', '%cd%\gvim_9.1.0821_x64.zip')"
powershell -Command "Expand-Archive -Path "%cd%\gvim_9.1.0821_x64.zip" -DestinationPath "%cd%" -Force"


powershell -Command "[System.Net.WebClient]::new().DownloadFile('https://github.com/skywind3000/vim-auto-popmenu/archive/refs/heads/master.zip', '%cd%\vim-auto-popmenu.zip')"
powershell -Command "Expand-Archive -Path "%cd%\vim-auto-popmenu.zip" -DestinationPath "%cd%" -Force"
robocopy /E /MOVE "%cd%\vim-auto-popmenu-master" "%cd%\vim\vimfiles" >nul

powershell -Command "[System.Net.WebClient]::new().DownloadFile('https://github.com/skywind3000/vim-dict/archive/refs/heads/master.zip', '%cd%\vim-dict.zip')"
powershell -Command "Expand-Archive -Path "%cd%\vim-dict.zip" -DestinationPath "%cd%" -Force"
robocopy /E /MOVE "%cd%\vim-dict-master" "%cd%\vim\vimfiles" >nul


(
echo let g:apc_enable_ft = {'*':1}
echo set cpt=.,k,w,b
echo set completeopt=menu,menuone,noselect
echo set shortmess+=c
echo let g:apc_min_length = 1
)>>"%cd%\vim\_vimrc"
